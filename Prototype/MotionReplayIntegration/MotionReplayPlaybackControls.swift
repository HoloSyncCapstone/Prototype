...
struct MotionReplayPlaybackControls: View {
    @ObservedObject var viewModel: MotionReplayViewModel
                ...
                VStack(alignment: .leading) {
                    ...
                    Picker("Session", selection: $viewModel.selectedSession) {
                        ForEach(viewModel.sessions) { session in
                            Text(session.name).tag(session as MotionSession?)
                        }
                    }
                    .pickerStyle(.menu)
                    .frame(width: 200)
            ...
            VStack(spacing: 20) { // Increased spacing
                ...
                if viewModel.showSubtitles && !viewModel.currentSubtitle.isEmpty {
                    Text(viewModel.currentSubtitle)
                        .font(.title) // Slightly smaller than extraLargeTitle to fit better
                        .fontWeight(.semibold)
