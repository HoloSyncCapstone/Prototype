...
struct MotionReplayAnimationView: View {
            ...
            RealityView { content, attachments in
                ...
                Task {
                    ...
                    if let dummyAvatar = try? await ModelEntity(named: "model_fV", in: .main) {
                        ...
                        if headFrames.count > 10 {
                            // Average over 10 frames to reduce noise
                            let dt = (headFrames[10].tMono - headFrames[0].tMono) / 10.0
                            if dt > 0.001 { // Avoid division by zero or extremely small dt
                                detectedFPS = 1.0 / dt
                                print("✅ Average FPS (10 frames): \(detectedFPS)")
                            }
                        } else if headFrames.count > 1 {
                            let dt = headFrames[1].tMono - headFrames[0].tMono
                            if dt > 0.001 {
                                detectedFPS = 1.0 / dt
                                print("✅ Detected FPS (single frame): \(detectedFPS)")
                            }
                        }
                        
                        // Clamp FPS to reasonable range to prevent "super fast" playback on noisy data
                        if detectedFPS > 120.0 {
                            print("⚠️ Detected FPS \(detectedFPS) is suspiciously high. Clamping to 60.0")
                            detectedFPS = 60.0
                        } else if detectedFPS < 10.0 {
                            print("⚠️ Detected FPS \(detectedFPS) is suspiciously low. Clamping to 30.0")
                            detectedFPS = 30.0
                        }
                ...
                Attachment(id: "controls") {
                    if viewModel.isLoading {
                        ...
                        MotionReplayPlaybackControls(viewModel: viewModel)
                    }
                }
            }
            .gesture(
